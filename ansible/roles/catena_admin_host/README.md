...
## Why it is a role rather than a file in roles/catena-admin
...
declaration could not act on anything: `roles/catena-admin/tasks/main.yml`
...
`roles/oauth2_proxy` resolves `catena_admin_hostname` by NAME at a point where
...
Immediately before `roles/catena-admin` in `site.yml`, which is where these
