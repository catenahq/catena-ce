...
def build_infra_doc(spec_path: Path, version_map: dict[str, dict],
    ...
    Returns None when the spec cannot be read. Returning a document with an
    EMPTY endpoint list instead would let the caller write it over the live
    config and restart Gatus on it -- so a missing spec file would silently
    delete every infrastructure monitor while the run reports success.
    Absent input must not produce output: None tells the caller to leave the
...
def main():
    ...
    # `gatus-<slug>` HC checks whose endpoint does not exist.
