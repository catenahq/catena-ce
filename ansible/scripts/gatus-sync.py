...
def build_infra_doc(spec_path: Path, version_map: dict[str, dict],
    ...
    Returns None when the spec cannot be read. Absent input must not produce
    output: a document with an EMPTY endpoint list is one the caller writes over
    the live config and restarts Gatus on, so a missing spec file deletes every
    infrastructure monitor while the run reports success. None tells the caller
    to leave the existing config alone.
    """
    header = (
